...
class TooltipRenderingDetails {
  ...
  void _renderTooltipView(Offset position, [int? seriesIndex]) {
    if (_stateProperties.chart is SfCartesianChart) {
      _renderCartesianChartTooltip(position, seriesIndex);
  ...
  void showTooltip(double? x, double? y, [int? seriesIndex]) {
    if (x != null &&
        y != null &&
        renderBox != null &&
      ...
      _mouseTooltip = false;
      isHovering ? _showMouseTooltip(x, y) : showTooltipView(x, y, seriesIndex);
    }
  }

  /// To show the chart tooltip.
  void showTooltipView(double x, double y, [int? seriesIndex]) {
    if (tooltipBehavior.enable &&
        renderBox != null &&
        _stateProperties.animationCompleted == true) {
      _renderTooltipView(Offset(x, y), seriesIndex);
  ...
  void showTemplateTooltip(Offset position,
      [int? currentSeriesIndex, dynamic xValue, dynamic yValue]) {
    ...
    if (_isPointWithInRect(
      ...
      for (int i = 0;
        ...
        if (seriesRendererDetails.visible! == true &&
            ...
            // The following condition is added to check whether the current series
            // index is equal to the given series index in showByIndex public method.
            (currentSeriesIndex == null ||
                currentSeriesIndex == seriesRendererDetails.seriesIndex)) {
      ...
      if (x != null && y != null && chart.series[seriesIndex].enableTooltip) {
        if (chart.tooltipBehavior.builder != null) {
          tooltipRenderingDetails.showTemplateTooltip(
              Offset(x, y), seriesIndex);
        } else if (chart.series[seriesIndex].enableTooltip) {
          tooltipRenderingDetails.showTooltip(x, y, seriesIndex);
        }
      }
  ...
  void _renderCartesianChartTooltip(Offset position, [int? seriesIndex]) {
    ...
    if (_isPointWithInRect(position, stateProperties.chartAxis.axisClipRect)) {
      ...
      for (int i = 0;
        ...
        if (_seriesRendererDetails!.visible! == true &&
            ...
            // index is equal to the given series index in showByIndex public method.
            (seriesIndex == null ||
                seriesIndex == _seriesRendererDetails!.seriesIndex)) {
          int count = 0;
